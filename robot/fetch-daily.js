  ...
  try {
    ...
    if (need.length){
      ...
      PENDING.forEach(p=>{
        ...
        let w = null;
        if (sf && sf.done){ if (sf.voided){ RECORD.unshift({ id:p.id, date:p.date, match:p.match, pick:p.pickLabel, odd:1.00, book:p.book, result:'V' }); return; } w = (sf.winnerHome === (p.pickKey==='home')); }
